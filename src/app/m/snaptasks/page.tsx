...
import { 
  ...
  MapPin, Clock, ArrowRight, X, Building2, User, Search
} from "lucide-react";
      ...
      <div className="px-4 py-2.5 bg-slate-900/10 border-b border-slate-900/60 z-10 shrink-0">
        <div className="relative flex items-center bg-slate-950 border border-slate-850 rounded-2xl px-3.5 py-2 flex-row focus-within:border-indigo-500/80 transition-all">
          ...
          <input 
            ...
            className="w-full bg-transparent outline-none text-xs font-semibold text-slate-200 placeholder-slate-600"
          />
          {searchTerm && (
            <button 
              type="button"
              onClick={() => setSearchTerm("")}
              className="p-1 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 shrink-0 transition-colors ml-1"
            >
