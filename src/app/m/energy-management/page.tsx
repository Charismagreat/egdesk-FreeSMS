...
import { useMobileEnergy } from "./hooks/useMobileEnergy";
import MobilePeakAlertCard from "./components/MobilePeakAlertCard";
import { Zap, CheckCircle, AlertOctagon, AlertTriangle, RefreshCw } from "lucide-react";

/**
 * 모바일 에너지 관제 메인 페이지
 */
export default function MobileEnergyManagementPage() {
  const {
    isLoading,
    toast,
    equipments,
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col justify-start w-full relative">
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-4 py-4 flex items-center justify-between sticky top-0 z-40 text-left">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-sm font-black text-white">에너지 관리 AI 모바일 관제</h1>
            <p className="text-[8px] text-slate-500 font-extrabold mt-0.5">EGDESK Smart Factory Energy Mobile</p>
          </div>
        </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto max-w-lg mx-auto w-full pb-16 space-y-5">
        ...
        {isLoading ? (
          <div className="py-24 text-center flex flex-col items-center justify-center space-y-3">
            ...
            <p className="font-bold text-slate-500 text-[10px]">실시간 에너지 관제 정보를 가져오는 중...</p>
          </div>
        ) : (
          <MobilePeakAlertCard
            equipments={equipments}
            loadPercentage={loadPercentage}
            isPeakRisk={isPeakRisk}
      ...
      <div className="py-4.5 text-center border-t border-slate-800 bg-slate-900 text-slate-600 text-[8px] font-black">
        © 2026 EGDESK SMART ENERGY. ALL RIGHTS RESERVED.
      </div>
