            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-sm text-slate-600">
                <th className="p-4 w-12 text-center">
                  <input type="checkbox" onChange={toggleSelectAll} checked={paginatedData.length > 0 && selectedIds.size === paginatedData.length} className="rounded" />
                </th>
              ...
              {paginatedData.length === 0 && (
                <tr>
                  <td colSpan={8} className="p-12 text-center text-slate-400">
                    {data.length === 0 ? "등록된 주문 내역이 없습니다." : "조건에 맞는 주문 내역이 없습니다."}
                  </td>
                </tr>
              )}
            </tbody>
