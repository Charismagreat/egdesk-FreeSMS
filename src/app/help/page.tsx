...
const FAQ_DATABASE: FAQItem[] = [
  ...
  },
  {
    ...
    answer: "네, 완벽히 이식되었습니다! PC 관제 대시보드에서 특정 태스크를 탭하여 상세 팝업을 열면, 우측 타임라인 피드 영역 바로 하단에 [PC 즉석 AI 스냅 입력 위젯]이 마운트되어 있습니다. 사장님이 모니터를 보면서 텍스트 기입은 물론 컴퓨터 속 이미지, 회의 오디오, 도면 PDF 파일을 즉시 드래그 앤 드롭 또는 선택 첨부하여 AI 자율 조치 스냅을 수행할 수 있으며, 단축키 Ctrl + Enter 로 즉석 전송 편리 기능까지 지원합니다."
  },
  {
    id: "order-10",
    category: "order",
  ...
  },
  {
    ...
    answer: "사장님이 F12 개발자 도구의 복잡한 CSS Selector나 특정 링크를 주입할 필요 없이, 경쟁사 품목명과 용량/수량 명세(예: '신라면', '120g 40개입')를 분리하여 자연어로만 기입하면 작동하는 최고급 편의 기능입니다. AI가 실시간 웹 검색망을 가동하여 가장 정합성이 높은 쿠팡, 네이버, 아마존/알리의 상품 후보 링크 3개를 추출하고, Playwright Stealth 브라우저로 봇 차단을 정밀 우회 접속하여 시세를 스캔합니다. 특히 사용자가 설정한 '규격/용량/수량'과 일치 여부를 Gemini AI가 2차 교차 매칭(Cross-Matching)하여 규격 오류 가격을 원천 차단하며, '감시 로봇 일괄 기동' 클릭 단 한번으로 DB에 즉각 등록 및 첫 수집 백필까지 완벽하게 완수 처리해 줍니다."
  },
  {
    id: "price-6",
    category: "price",
    question: "AI 자율 마이닝 시 탐색할 쇼핑몰이나 도메인을 직접 추가하거나 제어할 수 있나요?",
    answer: "네, 완벽하게 가능합니다! AI 자율 검색 입력창 바로 위에 마운트된 [탐색 대상 채널 제어] 제어판을 활용하면 됩니다. 기본 제공되는 4대 쇼핑몰(쿠팡, 네이버, 아마존, 알리)을 배지 칩 클릭만으로 On/Off 토글하여 제어할 수 있으며, 하단의 '채널 추가' 필드에 '지마켓', '11번가', '다나와' 등 사장님이 탐색하고 싶은 특정 포털 도메인을 즉석에서 추가 및 삭제하여 맞춤형으로 관리하실 수 있습니다. 추가된 커스텀 채널은 AI RAG 엔진에 동기화되어 맞춤형 크롤링 경로와 표준 Selector를 지능적으로 생성해 시세 수집 로봇으로 동시 배포하게 됩니다."
  }
];
