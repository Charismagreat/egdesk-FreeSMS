...
const FAQ_DATABASE: FAQItem[] = [
  ...
  },
  {
    id: "point-13",
    category: "point",
    question: "신용카드 사용내역 명세서 테이블에 새로 추가된 승인번호, 계정과목, 비고(지출 태그) 컬럼은 각각 어떻게 표출되며 어떤 비주얼 뱃지가 적용되나요?",
    answer: "이지데스크의 금융 관제 센터 내 신용카드 승인 내역 명세서 테이블에 **승인번호**, **계정과목 (대 〉 중 〉 소 분류)**, **비고 (지출 태그)** 컬럼이 신설되어 관제 수준이 대폭 향상되었습니다. 승인번호는 모노 폰트로 깔끔하게 노출되며, 계정과목은 DB의 실시간 매핑 데이터를 참조하여 은은한 그라디언트 파스텔 톤 뱃지(판관비는 초록, 제조원가는 파랑, 영업외비용은 노랑)와 함께 슬림한 화살표 기호(`〉`)로 3단 분류되어 가독성을 극대화합니다. 비고(지출 태그) 컬럼은 `tx.memo`에 저장된 쉼표 구분 태그 목록을 앰버 테마의 해시태그 칩으로 렌더링하여 지출 성격을 즉시 파악할 수 있도록 심미적 품격을 정밀하게 설계했습니다."
  },
  {
    id: "point-14",
    category: "point",
  ...
  },
  {
    id: "point-15",
    category: "point",
    question: "최고 관리자가 자연어로 지출 정산 규칙을 등록하는 'AI Rule Builder'는 무엇이며 어떻게 작동하나요?",
    answer: "최고 관리자 전용 'AI Rule Builder' 설정 카드 패널에 원하는 지출 정산 규칙을 친근한 한글 자연어(예: 'BC카드이고 금액 20만원 이하이며 사용일이 휴일이 아니고 오전 6시부터 오후 6시 사이에 차량번호 1234가 비고에 있다면 차량유지비로 분류해야 합니다.')로 기재하시면, 엔진이 이를 정밀 구조화(JSON)하여 DB에 저장합니다. 이후 실시간 RPA 배치 엔진이 부합하는 미확정 카드 거래 내역을 발굴하여 해당 계정과목으로 일괄 자동 정산하고, ⚡ [자연어 자동분류] 배지와 함께 적용된 자연어 규칙 툴팁을 동적으로 노출해 주는 지능형 정산 자율화 기능입니다."
  },
  {
    id: "point-16",
    category: "point",
