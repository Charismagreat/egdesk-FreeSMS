...
const FAQ_DATABASE: FAQItem[] = [
  ...
  },
  {
    ...
    answer: "최고 관리자 전용 '계정과목 AI Rule Builder' 설정 카드 패널에 원하는 지출 정산 규칙을 친근한 한글 자연어(예: 'BC카드이고 금액 20만원 이하이며 사용일이 휴일이 아니고 오전 6시부터 오후 6시 사이에 차량번호 1234가 비고에 있다면 차량유지비로 분류해야 합니다.')로 기재하시면, 엔진이 이를 정밀 구조화(JSON)하여 DB에 저장합니다. 이후 실시간 RPA 배치 엔진이 부합하는 미확정 카드 거래 내역을 발굴하여 해당 계정과목으로 일괄 자동 정산하고, ⚡ [자연어 자동분류] 배지와 함께 적용된 자연어 규칙 툴팁을 동적으로 노출해 주는 지능형 정산 자율화 기능입니다."
  },
  {
    id: "point-16",
    category: "point",
    question: "여러 개의 자연어 정산 규칙을 생성할 때 발생할 수 있는 룰 간의 중복이나 충돌은 어떻게 사전 예방되나요?",
    answer: "새 규칙을 등록할 때 기존의 활성 규칙들과 카드사, 카드번호, 휴일 여부, 시간대, 금액 범위, 비고 키워드가 겹쳐 '동일한 거래 건에 동시에 매칭될 수 있는 경합'이 포착되면 백엔드 충돌 감지 알고리즘(detectRuleConflict)이 실시간으로 경고를 생성합니다. UI 상에서 무조건 차단하는 대신 '기존의 활성 규칙 [...]과 매칭 조건이 대단히 유사하거나 중복되어 정산 경합이 발생할 수 있습니다. 그래도 등록하시겠습니까?' 라는 Confirm 경고창을 띄워 관리자가 실수를 미연에 감지 및 수정할 수 있게 돕고, 동의 시 force 플래그로 강제 최우선 적용할 수 있게 해 정산 논리 무결성을 철저히 지킵니다."
  },
  {
    id: "point-17",
    category: "point",
    question: "규칙을 적용하기 전에, 이 규칙이 실제로 내역에 어떤 영향을 줄지 미리 보고 백업받을 수 있나요?",
    answer: "네! 자연어 규칙 입력창 바로 옆에 배치된 '🔍 영향 건 미리보기'를 사용하시면 됩니다. 이를 클릭하면 백그라운드 시뮬레이션(Dry Run)이 구동되어 실제 데이터베이스를 갱신하지 않고서도 이 규칙에 부합하여 자동 분류될 예비 거래 건들을 테이블 형태로 화면에 먼저 표출해 줍니다. 또한 우측 상단의 '📥 CSV 다운로드' 버튼으로 이 예비 매칭 보고서 목록을 한글 깨짐 및 카드번호 깨짐이 완벽히 방지된 엑셀(CSV) 파일로 로컬에 소장 및 미리 백업해 보실 수 있습니다."
  },
  {
    id: "point-18",
    category: "point",
    question: "각 금융 정보(은행 계좌, 신용카드, 국세청 홈택스) 탭의 데이터 목록도 엑셀 다운로드가 가능한가요?",
    answer: "네, 물론입니다! 각 탭(은행 계좌 & 거래 내역, 법인 신용 카드 승인 내역, 국세청 홈택스 자료)의 테이블 우측 상단 필터 영역 왼쪽에 탑재된 에메랄드색 '📥 엑셀 다운로드' 버튼을 클릭하시면 됩니다. 사용자가 날짜나 키워드, 은행/카드사 등 교차 필터링을 지정하여 실시간으로 필터링한 명세를 UTF-8 BOM 인코딩 헤더(한글 깨짐 해결) 및 텍스트 안전 접두사(카드번호, 계좌번호, 승인번호 깨짐 방지)가 완벽 연동된 안전 엑셀 규격 파일로 실시간 초고속 내려받기 하실 수 있습니다."
  },
  {
    id: "point-19",
    category: "point",
    question: "다른 PC에서 깃허브 복사본으로 실행했더니 DB 오류가 발생합니다. 최초 데이터베이스 셋업은 어떻게 하나요?",
    answer: "깃허브에서 프로젝트를 처음 내려받아 새로운 PC에 이지데스크 환경을 구동하는 경우, 최초 1회 데이터베이스 테이블 빌드 및 최고관리자 생성을 해주셔야 합니다. [시스템 설정] 페이지 최상단에 마운트된 '데이터베이스 초기 설정 및 동기화' 패널에서 [DB 테이블 생성 및 동기화] 단추와 [기본 최고관리자 생성] 단추를 순서대로 클릭하시면 필요한 수십 개의 물리 테이블이 자동으로 생성되고 관리자 계정(admin / admin123)이 동기화되어 정상 작동합니다."
  },
  // 4. 쿠폰 & 주문/예약
  {
    id: "coupon-1",
  ...
  },
  {
    ...
    answer: "새롭게 설계된 풀스크린 마스터 테이블은 다중 선택 체크박스를 지원합니다. 테이블 좌측 체크박스를 선택하면 하단에 검은색 플로팅 '일괄 작업 바'가 자동으로 활성화되어, 선택된 대량의 견적서들을 한 번에 '일괄 발주서 전환' 하거나, 수주 내역들에 대해 '일괄 수주확인서 발송' 처리를 할 수 있으며, 일괄 엑셀 출력도 클릭 한 번으로 수행할 수 있습니다."
  },
  {
    id: "order-12",
    category: "order",
    question: "견적서 테이블에 표시되는 단가 오차 가이드는 어떤 정보인가요?",
    answer: "총 견적액 하단에 기준 품목 단가와의 오차율(예: +5% 💡)을 알려주어, 기존보다 비정상적으로 높게 또는 낮게 잡힌 단가 리스크를 직관적으로 상시 검수할 수 있습니다."
  },
  // 6. 가격 & 마진 추적 AI FAQ
  {
    id: "price-1",
  ...
  },
  {
    ...
    answer: "네! [피드백 관리] 메뉴에서 피드백 목록 좌측의 체크박스를 통해 여러 건을 다중 선택한 뒤, 하단에 나타나는 '실시간 피드백 옴니채널 발송 툴바'를 활용하시면 됩니다. 슬랙/디스코드 웹훅 연동은 물론, 대행사를 거치지 않고 직접 연동되는 '🌐 egdesk.cloud 사이트' 전송을 통해 Supabase 원격 DB와 원클릭 실시간 동기화가 가능합니다. 또한 '이메일 사전 문의(AI 초안)' 버튼을 누르면 AI가 정중한 제안 메일 초안을 자동 빌드해 주어, 검토 후 개발진(CHACHOGREAT@GMAIL.COM)으로 간편하게 전송할 수 있습니다."
  },
  {
    id: "sms-9",
    category: "sms",
    question: "이지봇 피드백 보드에서 많은 건수의 제보 데이터를 관리하는 팁이 있나요?",
    answer: "네! 많은 피드백이 누적되어도 안심하실 수 있도록 '서버사이드 페이지네이션'과 '실시간 필터링/검색 및 일괄 삭제' 시스템이 도입되어 있습니다. 우측 상단의 새로고침 버튼으로 실시간 데이터를 동기화하고, 하단 페이지네이션을 통해 10건씩 쾌적하게 쪼개어 조회할 수 있습니다. 특정 항목 우측의 '휴지통 🗑️' 아이콘을 눌러 단일 건을 삭제하거나, 목록 좌측 체크박스로 다중 선택한 뒤 하단 플로팅 툴바의 '🗑️ 선택 삭제' 버튼을 클릭해 한 번의 승인만으로 안전하게 일괄 영구 삭제할 수 있어 보드 관리가 매우 용이합니다."
  },
  {
    id: "rpa-6",
    category: "rpa",
  ...
  },
  {
    ...
    answer: "네! B2B 거래처 가입 시 발생할 수 있는 세무 리스크와 휴먼 에러를 방지하기 위해 **'로컬 체크섬 및 국세청 실시간 홈택스 연동 2중 보안 검증 쉴드'**가 완벽하게 마운트되어 있습니다. 사업자등록증(이미지/PDF) 스캔 완료 즉시 작동합니다: (1) **1차 로컬 체크섬 검사(Modulo 10)**: 등록번호 10자리가 수학적 규칙에 부합하는 유효 형식인지 로컬 단에서 즉각 검사합니다. (2) **2차 국세청 실시간 조회**: 국세청 홈택스 망과 실시간 동기화하여 해당 사업자의 현재 활동 상태를 '정상 가동중(계속사업자)', '휴업 사업자', '폐업 사업자', '미등록 사업자' 4대 상태로 지능형 스캔합니다. 만약 **폐업 사업자(🔴)**이거나 **미등록 가짜 번호(❌)**, 혹은 **수학적 오타(체크섬 오류)**가 감지되면, 하단의 거래처 등록 버튼이 자동으로 잠금(Lock/Disabled) 처리되어 부적격 사업자등록을 원천 방어합니다."
  },
  {
    id: "rpa-7",
    category: "rpa",
    question: "이지봇에게 사업자등록증 사진이나 PDF 파일을 전송해 B2B 거래처를 즉석에서 등록/갱신할 수 있나요?",
    answer: "네, 아주 완벽하고 똑똑하게 지원됩니다! 이지봇 대화창 아래 카메라(📷) 단추를 누른 뒤, 스캔할 사업자등록증 이미지 파일 또는 PDF(.pdf) 문서를 업로드해 보세요. AI가 자동으로 '사업자등록증'임을 1초 만에 감별하여 실시간 국세청 조회 및 기존 DB 대조 결과를 종합한 프리미엄 리포트 카드를 버블 메시지로 즉각 렌더링합니다. 검증을 거친 후 B2B 신규 바이어 등록 승인을 한 번에 끝낼 수 있습니다."
  },
  {
    id: "rpa-8",
    category: "rpa",
  ...
  },
  {
    ...
    answer: "이지데스크의 스마트 HR 3단계 기능으로, [근태 관리 AI] 페이지 최하단의 [임직원 상세 인적사항 명부 대장] 2열 보드에서 작동합니다. 사장님이 웹 화면의 좌측 폼에서 직원을 선택해 소속 부서, 공식 입사일, 거주지 및 교통수단 텍스트, 주요 보유 기술 태그, 그리고 비상 백업 담당자를 직접 입력하고 저장할 수 있습니다. 저장 즉시 우측의 상세 인적 명부 표와 SQLite 데이터베이스(`crm_operator_profiles`)에 실시간 Upsert 저장되며, 동시에 우측 실시간 AI 전사 업무 분석 예보(RAG) 엔진에 원본 데이터로 즉시 공급되어 기상 악화 시 지각 리스크나 대체 가능 인력 매핑 브리핑 리포트를 Gemini AI가 완벽히 재창출하게 됩니다."
  },
  {
    id: "sms-10",
    category: "sms",
    question: "시스템 설정의 [본사 및 관계사 설정] 기능은 무엇이며, RAG AI 비서에게 어떤 도움을 주나요?",
    answer: "본사(이 프로그램 사용자 회사)와 계열사/파트너사 등 관계사 정보를 등록하고 관리하는 기능입니다. 이를 등록해 두면 이지봇 AI 비서가 데이터베이스를 RAG(검색 증강 생성) 기반으로 분석할 때 본사/관계사명을 일반 외부 고객이나 타사 거래처로 잘못 인지하는 비즈니스 로직 오류를 완벽하게 방지합니다. 또한 본사 사업자등록증(이미지/PDF)을 업로드하면 Gemini OCR API가 상호, 대표자, 주소를 3초 만에 해독해 자동 입력해주며, 관계사 간 내부 거래 및 임직원 정보를 공유해 다각도의 전사적 브리핑을 가동할 수 있게 돕습니다."
  },
  {
    id: "point-12",
    category: "point",
    question: "인터넷뱅킹에서 다운로드받은 엑셀 파일을 금융정보 AI 페이지에 업로드하면 어떻게 매핑되어 저장되나요?",
    answer: "[금융정보 AI] 페이지의 수동 업로드 콘솔을 통해 각 금융사(IBK기업, 우리, 하나 등)의 입출금 거래내역 엑셀 파일(.xls, .xlsx)을 업로드할 수 있습니다. 업로드 즉시 백엔드의 transactionParser.js 스키마에 따라 각 금융사 고유의 칼럼 구조가 표준 금융 트랜잭션 포맷으로 자동 정규화 매핑되어 이지데스크 금융 DB(bank_transactions)에 Upsert 적재됩니다. 금융정보페이지에서 보여주는 통계 차트와 테이블은 이 적재된 실제 데이터베이스에서 실시간 쿼리하여 표출하므로 완벽한 금융 흐름 관제가 가능합니다."
  },
  {
    id: "price-7",
    category: "price",
    question: "국세청 홈택스에서 다운로드한 전자세금계산서 등의 엑셀 파일을 수동 업로드하여 연동할 수 있나요?",
    answer: "네, 적극 지원합니다! 홈택스에서 다운로드한 매출/매입 전자세금계산서, 면세 수수료 계산서, 현금영수증 내역 엑셀 파일을 업로드 콘솔에 드래그 앤 드롭하시면, 백엔드의 hometax-excel-parser.ts 엔진이 구문 파싱을 수행합니다. 파싱된 데이터는 SQLite 세무 DB에 멱등성이 유지된 상태로 자동 매핑 적재되어, 세금계산서 차감, 부가세 예측, AI 종합 마진 분석 리포트 및 이지봇 RAG 지식베이스에 실시간 반영되어 지능형 세무 상담을 가능하게 합니다."
  },
  {
    id: "order-11",
    category: "order",
    question: "지식 관리 AI 기능(Knowledge AI)에서 도면(CAD), 명함, 회의 녹취 오디오 등의 비정형 포맷은 어떻게 RAG 지식으로 적재되나요?",
    answer: "드래그 앤 드롭 업로드 즉시 각 포맷에 최적화된 AI 파이프라인이 가동됩니다. (1) CAD 도면은 도면 타이틀 블록을 감지해 설계자/프로젝트/BOM 리스트를 파싱하고 형상 특징 벡터를 추출합니다. (2) 명함은 Gemini OCR을 통해 상호, 연락처를 판독해 CRM 명함첩에 1:N 자동 적재하며 이직 히스토리까지 추적합니다. (3) 오디오 파일은 Whisper STT로 텍스트화하며, 본부장은 회의 결정사항과 5대 액션 아이템 보고서로 자동 리포팅 변환하여 마크다운 형태로 사내 RAG 지식베이스에 적재합니다."
  },
  {
    id: "order-12",
    category: "order",
    question: "문서 결재 및 지식 적재 시 적용되는 'Zero-Trust(제로 트러스트)' 보안 정책과 심사 하향이란 무엇인가요?",
    answer: "기안서가 승인되거나 자동 전결되는 즉시 보안 유출 사고 방지를 위해 무조건 'A등급 (최고 기밀)' 상태로 강제 지정되어 격리 적재됩니다. 이 시점에서는 오직 최고관리자 및 대표이사만 질문이나 RAG 검색으로 해당 내용 조회가 가능합니다. 이후 최고관리자가 대시보드 상단에 활성화된 '기밀 문서 심사 피드'를 통해 실시간 심사를 승인하고 등급을 B(부서 대외비) 또는 C(사내 공개)로 하향 조정(Downgrade)하는 즉시, 새로고침 없이 실시간으로 전사 임직원의 RAG 챗봇 검색 가용망으로 방출 노출됩니다."
  },
  {
    id: "order-13",
    category: "order",
    question: "AI 결재 자동 파일럿(Autopilot) 전결 기능의 승인 판단 기준과 안전 제동 장치는 어떻게 동작하나요?",
    answer: "청구 금액이 10만 원 이하의 소액이고 서식 일치도 및 예산 한도 준수가 95.0점 이상을 만족하는 정형 기안에 대해, 인간 결재자의 승인 대기 없이 시스템이 즉각 'APPROVED_AUTO(자동 전결)'를 처리합니다. 다만 예산 초과나 중복 기안, 대외비 분류 등 이상 징후 감지 시 파일럿이 즉각 정지되며 즉각 수동 심사선(부서장)으로 안전 Failover 처리됩니다. 자동 승인 시에도 AI가 왜 전결을 결정했는지 요약 근거가 감사 이력 코멘트에 투명하게 보관됩니다."
  }
];
