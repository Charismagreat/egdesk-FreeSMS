...
async function initComprehensiveProfilesDatabase() {
  ...
  // 1. 학력사항 (crm_operator_education)
  await queryTable('crm_operator_education', { limit: 1 }).catch(async () => {
    console.log('crm_operator_education 테이블 생성 중...');
    await createTable('임직원 학력이력 대장', [
    ...
    await insertRows('crm_operator_education', demoEdu);
  });

  // 2. 자격면허 (crm_operator_licenses)
  await queryTable('crm_operator_licenses', { limit: 1 }).catch(async () => {
    console.log('crm_operator_licenses 테이블 생성 중...');
    await createTable('임직원 자격면허 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
    ...
    await insertRows('crm_operator_licenses', demoLic);
  });

  // 3. 이전경력 (crm_operator_careers)
  await queryTable('crm_operator_careers', { limit: 1 }).catch(async () => {
    console.log('crm_operator_careers 테이블 생성 중...');
    await createTable('임직원 이전경력 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
    ...
    await insertRows('crm_operator_careers', demoCar);
  });

  // 4. 급여상여 (crm_operator_salaries)
  await queryTable('crm_operator_salaries', { limit: 1 }).catch(async () => {
    console.log('crm_operator_salaries 테이블 생성 중...');
    await createTable('임직원 급여상여 이력 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
    ...
    await insertRows('crm_operator_salaries', demoSal);
  });

  // 5. 부서이동/승진 (crm_operator_promotions)
  await queryTable('crm_operator_promotions', { limit: 1 }).catch(async () => {
    console.log('crm_operator_promotions 테이블 생성 중...');
    await createTable('임직원 승진발령 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
    ...
    await insertRows('crm_operator_promotions', demoPro);
  });

  // 6. 상벌이력 (crm_operator_awards)
  await queryTable('crm_operator_awards', { limit: 1 }).catch(async () => {
    console.log('crm_operator_awards 테이블 생성 중...');
    await createTable('임직원 상벌 징계 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
    ...
    await insertRows('crm_operator_awards', demoAwd);
  });

  // 7. 경조사이력 (crm_operator_family_events)
  await queryTable('crm_operator_family_events', { limit: 1 }).catch(async () => {
    console.log('crm_operator_family_events 테이블 생성 중...');
    await createTable('임직원 경조사 지원 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
      { name: 'event_date', type: 'TEXT', notNull: true },
      { name: 'relation', type: 'TEXT', notNull: true }, // 본인, 부친, 모친, 배우자 등
      { name: 'type', type: 'TEXT', notNull: true }, // 결혼, 장례, 출산 등
      { name: 'congratulation_money', type: 'REAL', defaultValue: 0 },
      { name: 'wreath_provided', type: 'INTEGER', defaultValue: 0 } // 1: 제공, 0: 미제공
    ], {
    ...
    await insertRows('crm_operator_family_events', demoFev);
  });

  // 8. 병력치료 (crm_operator_medical)
  await queryTable('crm_operator_medical', { limit: 1 }).catch(async () => {
    console.log('crm_operator_medical 테이블 생성 중...');
    await createTable('임직원 병력 치료 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
    ...
    await insertRows('crm_operator_medical', demoMed);
  });

  // 9. 대내외 사건사고 [NEW] (crm_operator_incidents)
  await queryTable('crm_operator_incidents', { limit: 1 }).catch(async () => {
    console.log('crm_operator_incidents 테이블 생성 중...');
    await createTable('임직원 대내외 사건사고 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
      { name: 'occurred_date', type: 'TEXT', notNull: true },
      { name: 'severity', type: 'TEXT', notNull: true }, // LOW, MEDIUM, HIGH
      { name: 'title', type: 'TEXT', notNull: true },
      { name: 'description', type: 'TEXT', notNull: true },
      { name: 'status', type: 'TEXT', notNull: true }, // 진행중, 합의완료, 종결
    ...
    await insertRows('crm_operator_incidents', demoInc);
  });

  // 10. 다차원 평판 [NEW] (crm_operator_reputations)
  await queryTable('crm_operator_reputations', { limit: 1 }).catch(async () => {
    console.log('crm_operator_reputations 테이블 생성 중...');
    await createTable('임직원 다차원 평판 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
      { name: 'evaluation_date', type: 'TEXT', notNull: true },
      { name: 'evaluator_id', type: 'TEXT', defaultValue: '익명' },
      { name: 'source_type', type: 'TEXT', notNull: true }, // INTERNAL (동료), EXTERNAL (바이어/고객), MANAGER (부서장)
      { name: 'score', type: 'REAL', notNull: true }, // 1~5
    ...
    await insertRows('crm_operator_reputations', demoRep);
  });

  // 11. 부양가족 [NEW] (crm_operator_families)
  await queryTable('crm_operator_families', { limit: 1 }).catch(async () => {
    console.log('crm_operator_families 테이블 생성 중...');
    await createTable('임직원 부양가족 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
      { name: 'relation_type', type: 'TEXT', notNull: true }, // 배우자, 자녀, 부친, 모친, 형제자매 등
      { name: 'name', type: 'TEXT', notNull: true },
      { name: 'birth_date', type: 'TEXT', notNull: true }, // YYYY-MM-DD
      { name: 'phone_number', type: 'TEXT', notNull: true },
      { name: 'is_dependent', type: 'INTEGER', defaultValue: 0 }, // 1: 부양, 0: 비부양
      { name: 'remarks', type: 'TEXT', defaultValue: '없음' }
    ], {
    ...
    await insertRows('crm_operator_families', demoFam);
  });

  // 12. 담당업무 변경이력 (crm_operator_job_history)
  await queryTable('crm_operator_job_history', { limit: 1 }).catch(async () => {
    console.log('crm_operator_job_history 테이블 생성 중...');
    await createTable('임직원 담당업무 변경이력 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
      ...
      { name: 'is_current', type: 'INTEGER', defaultValue: 1 } // 1: 현재, 0: 과거
    ], {
    ...
    await insertRows('crm_operator_job_history', demoJob);
  });

  // 13. 참여 프로젝트 (crm_operator_projects)
  await queryTable('crm_operator_projects', { limit: 1 }).catch(async () => {
    console.log('crm_operator_projects 테이블 생성 중...');
    await createTable('임직원 참여 프로젝트 대장', [
      { name: 'id', type: 'TEXT', notNull: true },
      { name: 'operator_id', type: 'TEXT', notNull: true },
      ...
      { name: 'contribution_rate', type: 'REAL', defaultValue: 100 }, // 0 ~ 100 %
      { name: 'performance_score', type: 'REAL', defaultValue: 0 }, // 0 ~ 100
    ...
    await insertRows('crm_operator_projects', demoPrj);
  });
}
