...
export async function PUT(request: NextRequest) {
  try {
    ...
    // 🔑 3. 로컬 SQLite DB (financehub.db)에서 은행 거래 내역 업데이트
    ...
    console.log(`[Bank Update API] Target DB found: ${targetPath}`);
    ...
    // category와 memo를 bank_transactions 테이블에 저장합니다.
    ...
    if (info.changes === 0) {
      return NextResponse.json(
        { success: false, error: "지정된 ID의 은행 거래 내역이 존재하지 않거나 변경사항이 없습니다." },
        { status: 404 }
      );
    }
